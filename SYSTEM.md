...
- **2026-07-21 (h)** — Descarga de gráfico en **HD**: `pixelRatio` automático (según `getWidth/getHeight`). Objetivo: **lado largo ≥ 2048 px** y corto ≥ 1280 px (para que WhatsApp ofrezca el toggle "HD", que aparece sobre ~1600 px). Título/subtítulo escalados. Archivo `..._HD.png`.
- **2026-07-21 (g)** — Descargar gráfico como imagen ahora **compone título + subtítulo** (del `h3` y `.hint`) sobre el PNG con canvas. Nuevo `COWORK_ACTUALIZACION.md` (instrucciones para que Cowork actualice vía GitHub Pages y borre la tarea vieja de Netlify).
- **2026-07-21 (f)** — (1) **Fix ventana de barras**: `hbz` mostraba ~25 barras en listas grandes por un piso de 5% en el zoom; ahora muestra ~10 y el resto se recorre con el slider (Top clientes/productos/unidades window=10). (2) **Descargar gráfico como imagen** (`injectDownloads`: botón ⬇ en cada tarjeta con gráfico, usa `getDataURL`). (3) **Pestaña Comparar** (`comp`): dos rangos de fecha → tabla de indicadores A vs B con Δ + barra. (4) **Clientes inactivos** (`renderInactive`): card en pestaña Clientes, sin compras hace >45 días, sobre toda la data.
